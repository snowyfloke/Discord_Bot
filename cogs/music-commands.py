...
def build_queue_pages(
    queue, per_page=20
):  # Change this line if you wish to have more songs on each page!
    pages = []
    for i in range(0, len(queue), per_page):
        chunk = queue[i : i + per_page]
        lines = [
            f"`{i + j + 1}.` {title}"
            for j, (stream_url, title, yt_url) in enumerate(chunk)
        ]
        pages.append("\n".join(lines))
    return pages
...
class Music(commands.Cog):
        ...
        if ctx.author.voice is None:
            ...
            if ctx.voice_client is None:
                ...
                try:
                    ...
                    msg = get_msg(lang, "bot_joined_vc").format(
                        channel_name=channel.name
                    )
                    await ctx.send(msg)
                except Exception as e:
                    msg = get_msg(lang, "error").format(error_msg=e)
                    await ctx.send(msg)
                    return

            flat_tracks = await asyncio.get_event_loop().run_in_executor(
                None, lambda: get_flat_entries(query)
            )
            for entry in flat_tracks:
                queue.append((None, entry[1], entry[0]))

            if len(flat_tracks) > 1:
                total_songs = len(flat_tracks)
                msg = get_msg(lang, "multiple_songs_added_to_queue").format(
                    quantity=total_songs
                )
                await ctx.send(msg)
            else:
                current_track = flat_tracks[0][1]
                msg = get_msg(lang, "song_added_to_queue").format(
    ...
    async def pause(self, ctx):
        ...
        if ctx.voice_client is None:
            ...
            msg = (
                ...
                if lang == "pt"
                else "You're not in a voice channel..."
            )
            await ctx.send(msg)
            return
        elif ctx.voice_client.is_paused():
            msg = (
                "A música já estava pausada..."
                if lang == "pt"
                else "The song was already paused..."
            )
            await ctx.send(msg)
            return
    ...
    async def resume(self, ctx):
        ...
        if ctx.voice_client is None:
            ...
            msg = (
                ...
                if lang == "pt"
                else "The song was already playing..."
            )
            await ctx.send(msg)
    ...
    async def stop(self, ctx):
        ...
        if ctx.voice_client is None:
            ...
            msg = (
                "Parei a reprodução :)" if lang == "pt" else "I stopped the playback :)"
            )
            await ctx.send(msg)
    ...
    async def skip(self, ctx):
        ...
        if ctx.voice_client.is_playing():
            ...
            msg = (
                "Pulei a música atual!" if lang == "pt" else "Skipped the current song!"
            )
            await ctx.send(msg)
